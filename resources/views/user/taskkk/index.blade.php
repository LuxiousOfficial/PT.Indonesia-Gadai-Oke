<x-layout-user>
  @if (session()->has('success'))
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      <h1 class="fs-3 fw-bold text-warning text-capitalize">Data Task 3</h1>
    </div>
    ...
    <section class="bg-white rounded-3 p-4">
      ...
      <div class="row">
        <div class="col-md-8 col-12">
          <a href="/user/taskkk/create" class="btn btn-warning mb-3 fw-bold">Add Task</a>
        </div>
      </div>
      @endcan
          ...
          <table class="table table-sm">
            ...
            <tbody>
              ...
              <tr class="text-center text-black fw-bold">
                  ...
                  <td class="d-flex">
                    <a href="/user/taskkk/{{ $task->id }}" class="btn btn-info px-3 fw-bold border-0">Details</a>
                    <a href="/user/taskkk/{{ $task->id }}/edit" class="btn btn-warning px-3 fw-bold border-0 mx-2">Edit</a>
                    @can('manager')
                    <form method="post" action="/user/taskkk/{{ $task->id }}">
                      @method('delete')
                      @csrf
